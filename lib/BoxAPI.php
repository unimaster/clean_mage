...
class BoxAPI {
	...
	/* Saves the token */
	public function write_token($token, $type = 'file',$getprofileid) {
		...
		if(isset($array['error'])){
			...
			if($type == 'file'){
				...
				$filerename='/token_'.$getprofileid.'.box';
				$fp = fopen($dirPath.$filerename, 'w');
	...
	/* Reads the token */
	public function read_token($type = 'file', $json = false,$getprofileid) {
		$dirPath = Mage::getBaseDir().DS.'var'.DS.'data';
		$filerename='/token_'.$getprofileid.'.box';

		if($type == 'file' && file_exists($dirPath.$filerename)){ 
			$fp = fopen($dirPath.$filerename, 'r');
			$content = fread($fp, filesize($dirPath.$filerename));
			fclose($fp);
		} else {
			return false;
		}
	...
	/* Loads the token */
	public function load_token($getprofileid) {
		$array = $this->read_token('file','',$getprofileid);
		...
		if(!$array){
			...
			if(isset($array['error'])){
				...
				if($this->write_token($token, 'file',$getprofileid)){
					$array = json_decode($token, true);
					$this->refresh_token = $array['refresh_token'];
